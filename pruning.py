...
def train(epoch):
    ...
    for batch_idx, (data, targets) in pbar:
        ...
        if batch_idx % args.log_interval == 0:
            ...
            percentage = 100. * batch_idx / len(train_loader) 
            #(train_loss/batch_idx+1).3f
            pbar.set_description(f'Train Epoch: {epoch} [{done:5}/{len(train_loader.dataset)} ({percentage:3.0f}%)] loss: {train_loss:.6f}')
...
def test():
    ...
    with torch.no_grad():
        for batch_idx, (data, targets) in enumerate(test_loader):
            data, targets = model.to(device), targets.to(device)
            outputs = model(data)
            loss = F.nll_loss(outputs, targets)
